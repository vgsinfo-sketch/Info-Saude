...
import { Heart, AlertTriangle, Pill, Phone, User, ShieldAlert, Activity, Info, CreditCard, Calendar, Printer, Download, Loader2, Paperclip, FileText, ExternalLink } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans print:bg-white selection:bg-blue-100">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @media print {
          ...
          /* Ajustar textos */
          .text-4xl, .text-5xl { font-size: 18pt !important; color: black !important; font-weight: bold !important; }
          .text-3xl { font-size: 15pt !important; color: black !important; font-weight: bold !important; }
          .text-2xl { font-size: 13pt !important; color: black !important; }
            ...
            <div>
              ...
              {userData.data_validade && (
                <p className={`text-[9px] font-bold uppercase tracking-[0.2em] mt-1 ${
                  ...
                  {new Date(userData.data_validade) >= new Date() 
                    ? `✓ Cadastro Válido até ${new Date(userData.data_validade).toLocaleDateString('pt-BR')}`
                    : `⚠ Cadastro Expirado em ${new Date(userData.data_validade).toLocaleDateString('pt-BR')}`
                  }
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center md:items-start relative z-10">
              <div className="relative shrink-0">
                <div className="w-32 h-32 md:w-40 md:h-40 bg-slate-900 rounded-[2rem] flex items-center justify-center text-5xl md:text-6xl font-black text-white shadow-2xl shadow-slate-300 border-[8px] border-white font-display italic overflow-hidden">
                  {userData.foto ? (
                    <img src={userData.foto} alt={userData.nome_completo} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  ) : (
